...
def subscribe_tvs(cursor):
    ...
    for title, season, total_episodes, year, douban_id, tmdb_id, status in rss_tvs:
        ...
        # 修复：优先用 tmdb_id（或 douban_id）查询，避免 title 被改写后查不到导致重复插入
        # 查询时同时取 id，后续 UPDATE/DELETE 统一用 id 定位
        if tmdb_id:
            miss_row = cursor.execute(
                'SELECT id, missing_episodes FROM MISS_TVS WHERE tmdb_id = ?', (tmdb_id,)
            ).fetchone()
        elif douban_id:
            miss_row = cursor.execute(
                'SELECT id, missing_episodes FROM MISS_TVS WHERE douban_id = ?', (douban_id,)
            ).fetchone()
        else:
            miss_row = cursor.execute(
                'SELECT id, missing_episodes FROM MISS_TVS WHERE title = ? AND year = ? AND season = ?',
                (title, year, season)
            ).fetchone()
        miss_id = miss_row[0] if miss_row else None
        miss_episodes_val = miss_row[1] if miss_row else None
        ...
        if not lib_exists:
            ...
            if not miss_id:
                ...
                subscribed_missing = set(int(ep) for ep in miss_episodes_val.split(',') if ep.strip().isdigit()) if miss_episodes_val else set()
                ...
                if len(total_episodes_set) != len(subscribed_missing):
                    ...
                    cursor.execute(
                        'UPDATE MISS_TVS SET missing_episodes = ? WHERE id = ?',
                        (new_missing_episodes_str, miss_id)
                    )
            ...
            # 部分或全部已入库的情况
            # 优先用 tmdb_id 查询已存在的集数，无 tmdb_id 时回退到 actual_title 匹配
            # 修复：原代码用 actual_title 查 LIB_TVS.title，标题不一致时返回空导致误判全部缺失
            if tmdb_id:
                existing_episodes_str = cursor.execute(
                    '''SELECT episodes FROM LIB_TV_SEASONS WHERE tv_id = (SELECT id FROM LIB_TVS WHERE tmdb_id = ?) AND season = ?''',
                    (tmdb_id, actual_season)
                ).fetchone()
            else:
                existing_episodes_str = cursor.execute(
            ...
            if miss_id and miss_episodes_val:
                subscribed_missing = set(int(ep) for ep in miss_episodes_val.split(',') if ep.strip().isdigit())
            else:
...
def update_subscriptions(cursor):
    """检查并更新当前订阅 - 支持别名关联，优先用 tmdb_id 匹配 LIB 表"""
    # 检查并删除已入库的电影
    cursor.execute('SELECT title, year, tmdb_id FROM MISS_MOVIES')
    miss_movies = cursor.fetchall()

    for title, year, tmdb_id in miss_movies:
        # 优先用 tmdb_id 匹配 LIB_MOVIES，无 tmdb_id 时回退到 title+year
        if tmdb_id:
            lib_match = cursor.execute('SELECT 1 FROM LIB_MOVIES WHERE tmdb_id = ?', (tmdb_id,)).fetchone()
        else:
            lib_match = cursor.execute('SELECT 1 FROM LIB_MOVIES WHERE title = ? AND year = ?', (title, year)).fetchone()
        if lib_match:
            if tmdb_id:
                cursor.execute('DELETE FROM MISS_MOVIES WHERE tmdb_id = ?', (tmdb_id,))
            else:
    ...
    # 检查并删除已完整订阅的电视剧
    cursor.execute('SELECT id, title, year, season, missing_episodes, tmdb_id FROM MISS_TVS')
    miss_tvs = cursor.fetchall()

    for record_id, title, year, season, missing_episodes, tmdb_id in miss_tvs:
        # 检查是否存在别名关联
        alias_row = cursor.execute(
            'SELECT TARGET_TITLE, TARGET_SEASON FROM LIB_TV_ALIAS WHERE ALIAS = ?', 
            (title,)
        ...
        # 优先用 tmdb_id 查询 LIB_TVS 的 id，无 tmdb_id 时回退到 title 匹配
        if tmdb_id:
            tv_row = cursor.execute('SELECT id FROM LIB_TVS WHERE tmdb_id = ?', (tmdb_id,)).fetchone()
        else:
            tv_row = cursor.execute('SELECT id FROM LIB_TVS WHERE title = ?', (actual_title,)).fetchone()
...
def update_alias_subscriptions(cursor):
    ...
    cursor.execute('''
        SELECT mt.id, mt.title, mt.season, mt.missing_episodes, mt.year, mt.tmdb_id, lta.TARGET_TITLE, lta.TARGET_SEASON
        FROM MISS_TVS mt
    ...
    for record_id, alias_title, alias_season, missing_episodes, year, miss_tmdb_id, target_title, target_season in alias_records:
        # 优先用 tmdb_id 查找目标剧集，无 tmdb_id 时回退到 target_title 匹配
        # 修复：原代码仅用 target_title 匹配，媒体库标题变化后别名订阅无法清理
        if miss_tmdb_id:
            lib_tv_row = cursor.execute(
                'SELECT id FROM LIB_TVS WHERE tmdb_id = ?', (miss_tmdb_id,)
            ).fetchone()
        else:
            lib_tv_row = cursor.execute(
                    ...
                    if season_response.status_code == 200:
                        ...
                        if missing_episodes:
                            try:
                                ...
                                # 修复：优先用刚获取的 tmdb_id 查 LIB_TVS，避免标题不一致导致查询失败
                                existing_episodes_str = cursor.execute(
                                    '''SELECT episodes FROM LIB_TV_SEASONS WHERE tv_id = (SELECT id FROM LIB_TVS WHERE tmdb_id = ?) AND season = ?''',
                                    (tmdb_id, season)
                                ).fetchone()
